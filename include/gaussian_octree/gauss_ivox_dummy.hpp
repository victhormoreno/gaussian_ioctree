...
public:
    ...
    std::size_t points_since_update = 0;
...
// private:
    ...
    void computeGeometry(UnionFindNode* node) {
        ...
        // Covariance from scatter matrix: E[X^2] - E[X]^2
        ...
        Scalar l1 = std::max(evals(1), 0.0);
        Scalar l2 = std::max(evals(2), 0.0);

        Scalar linearity  = (l2 - l1) / l2;
        Scalar planarity  = (l1 - l0) / l2;
        Scalar scattering = l0 / l2;
        
        Scalar LINE_THRESHOLD = 0.75; // Require linearity to be highly dominant
        ...
        else if (linearity > planarity && linearity > scattering && linearity > LINE_THRESHOLD) {
    ...
    void tryMerge(UnionFindNode* a, UnionFindNode* b) {
        ...
        switch (ga->type) {
            case PrimitiveType::PLANE: {
                ...
                Mat3 total_cov = ga->plane_cov + gb->plane_cov;
                total_cov.diagonal().array() += 0.0001; // 1cm regularization floor

                Scalar m_dist_sq = delta_pi.transpose() * total_cov.inverse() * delta_pi;

                if (m_dist_sq < 7.81)
                    merge = true; // 7.81 is chi-square for 3DOF
            ...
            case PrimitiveType::LINE: {
                ...
                Scalar max_dist = v_size_; // threshold can be tuned
            ...
            case PrimitiveType::VOLUME: {
                ...
                Scalar max_mdist = v_size_; // threshold can be tuned
